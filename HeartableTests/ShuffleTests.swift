...
final class ShuffleTests: XCTestCase {
    @MainActor
    func testRetiredWeightedPreferenceMigratesToOrdinaryShuffle() {
        let suite = "ShuffleTests.migration.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!
        defer { defaults.removePersistentDomain(forName: suite) }
        defaults.set("weighted", forKey: "heartable_shuffle_mode")
        let prefs = PlaybackPrefsStore(defaults: defaults)
        XCTAssertEqual(prefs.mode, .shuffle)
        XCTAssertEqual(defaults.string(forKey: "heartable_shuffle_mode"), "shuffle")
        prefs.cycleMode()
        XCTAssertEqual(prefs.mode, .order)
        prefs.cycleMode()
        XCTAssertEqual(prefs.mode, .shuffle)
        XCTAssertEqual(ShuffleMode.allCases, [.order, .shuffle])
    }

    func testOrderModePreservesInput() {
        let input = ["one", "two", "three"]
        XCTAssertEqual(orderForPlayback(input, mode: .order), input)
    }

    func testShuffleModesReturnEveryTrackExactlyOnce() {
        let input = Array(0..<100).map(String.init)

        for mode in [ShuffleMode.shuffle] {
