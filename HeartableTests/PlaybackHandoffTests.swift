...
final class PlaybackHandoffTests: XCTestCase {
    func testAcceptedButIdlePlaybackRequestsNativeHandoff() async {
        var polls = 0
        do {
            _ = try await PlaybackStartupRetry.confirmSpotifyPlayback(uri: "spotify:track:new",
                deviceID: nil, delay: .zero) { polls += 1; return .idle }
            XCTFail("Accepted Play without actual playback must wake Spotify")
        } catch { XCTAssertTrue(error is NoActiveDeviceError) }
        XCTAssertEqual(polls, 3)
    }

    func testPausedSongOrWrongSongCannotCountAsSuccessfulPlayback() async throws {
        for (playing, uri) in [(false, "spotify:track:new"), (true, "spotify:track:old")] {
            let state = try JSONDecoder().decode(PlaybackState.self, from: Data("""
            {"is_playing":\(playing),"shuffle_state":false,"repeat_state":"off",
             "item":{"id":"new","name":"Song","uri":"\(uri)"}}
            """.utf8))
            do {
                _ = try await PlaybackStartupRetry.confirmSpotifyPlayback(uri: "spotify:track:new",
                    deviceID: nil, attempts: 1, delay: .zero) { .state(state) }
                XCTFail("Playlist settings cannot stand in for actual playback")
            } catch { XCTAssertTrue(error is NoActiveDeviceError) }
        }
    }

    func testConfirmedSongDoesNotNeedShuffleSettingsToProvePlayback() async throws {
        let state = try JSONDecoder().decode(PlaybackState.self, from: Data("""
        {"is_playing":true,"shuffle_state":true,"repeat_state":"context",
         "device":{"id":"phone","is_active":true},"item":{"id":"new","name":"Song","uri":"spotify:track:new"}}
        """.utf8))
        let confirmed = try await PlaybackStartupRetry.confirmSpotifyPlayback(uri: "spotify:track:new",
            deviceID: "phone", delay: .zero) { .state(state) }
        XCTAssertEqual(confirmed.item?.uri, "spotify:track:new")
    }

    func testPlaybackConfirmationHonorsRateLimitWithoutOpeningSpotify() async {
        do {
            _ = try await PlaybackStartupRetry.confirmSpotifyPlayback(uri: "spotify:track:new",
                deviceID: nil, delay: .zero) { .rateLimited(60) }
            XCTFail("Expected rate limit")
        } catch {
            XCTAssertFalse(error is NoActiveDeviceError)
            XCTAssertFalse(error is SpotifyPlaybackRestrictedError)
        }
    }

    func testNativeStartIsPublishedBeforeQueueAndSurvivesQueueRefusal() async throws {
        var events: [String] = []
        try await PlaybackStartupRetry.finishNativeHandoff(
            wake: { events.append("wake") },
            started: { events.append("playing") },
            configureQueue: {
                XCTAssertEqual(events, ["wake", "playing"])
                events.append("queue")
                throw NoActiveDeviceError()
            },
            queueFailed: { events.append("queue warning") }
        )
        XCTAssertEqual(events, ["wake", "playing", "queue", "queue warning"])
    }

    func testFailedNativeWakeDoesNotClaimPlaybackOrInstallQueue() async {
        do {
            try await PlaybackStartupRetry.finishNativeHandoff(
                wake: { throw ProviderError("Spotify unavailable") },
                started: { XCTFail("No confirmed start") },
                configureQueue: { XCTFail("No connected player") },
                queueFailed: { XCTFail("Must surface the actual start failure") }
            )
            XCTFail("Expected start failure")
        } catch { XCTAssertEqual(error.localizedDescription, "Spotify unavailable") }
    }

    func testCancelledNativeQueueDoesNotPublishFailureFeedback() async {
        do {
            try await PlaybackStartupRetry.finishNativeHandoff(
                wake: {}, started: {},
                configureQueue: { throw CancellationError() },
                queueFailed: { XCTFail("An obsolete request must stay silent") }
            )
