...
final class AccountIsolationTests: XCTestCase {
    ...
    func testMasterLibrarySnapshotReadsOnlyTheExplicitAccountFile() async {
        let first = UUID()
        let second = UUID()
        let snapshot = MasterLibrarySnapshot(tracks: [], artists: [])
        await snapshot.save(ownerID: first)
        ...
        let own = await MasterLibrarySnapshot.load(ownerID: first)
        let other = await MasterLibrarySnapshot.load(ownerID: second)
        XCTAssertNotNil(own)
        XCTAssertNil(other)
    }
