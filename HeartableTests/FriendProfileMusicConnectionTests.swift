...
final class FriendProfileMusicConnectionTests: XCTestCase {
    ...
    func testMixtapeCanStartWithoutATitlePrompt() async throws {
        let id = UUID()
        let creator = FriendMixtapeCreator(create: { title, recipient in
            XCTAssertEqual(title, "Untitled mixtape")
            XCTAssertEqual(recipient, self.friendID)
            return id
        })
        let created = try await creator.createDraft(friendID: friendID)
        XCTAssertEqual(created, id)
    }

    func testSongInsertionRetainsRetryIdentityInPayload() throws {
        let id = UUID()
        let payload = MixtapeTrackInsertDTO(id: id, mixtapeId: UUID(), position: 0,
            trackUri: "spotify:track:test", trackName: "Test", artist: nil, albumArt: nil, durationMs: nil)
        let decoded = try JSONDecoder().decode(MixtapeTrackInsertDTO.self, from: JSONEncoder().encode(payload))
