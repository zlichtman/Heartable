...
final class LibrarySessionStoreTests: XCTestCase {
    ...
    @MainActor
    func testInactiveSceneDefersOptionalLibraryWorkForResume() async {
        let session = LibrarySessionStore()
        session.setActive(false)
        await session.synchronize(providers: [], playlistTracks: PlaylistTracksRepository(persistenceEnabled: false))
        XCTAssertTrue(session.needsSynchronization)
        XCTAssertFalse(session.synchronizing)
        session.reset()
        XCTAssertFalse(session.needsSynchronization)
    }
