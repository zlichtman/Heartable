...
final class VinylShelfTests: XCTestCase {
    ...
    private func render(themeKey: String, selection: Int, withChrome: Bool = false,
        ...
        if withChrome {
            ...
            XCTAssertLessThanOrEqual(frame.maxY, try XCTUnwrap(playback.geometry.frame).minY,
                "Shelf must end above player: shelf=\(frame), player=\(String(describing: playback.geometry.frame))")
        }
        if startPlayback {
            ...
            for _ in 0..<5 {
                for target in [CGSize(width: 390, height: 844), CGSize(width: 844, height: 390),
                    ...
                    if target.width > target.height {
                        ...
                        let frame = try XCTUnwrap(playback.shelfFrame)
                        XCTAssertLessThanOrEqual(frame.maxY, try XCTUnwrap(playback.geometry.frame).minY,
                            "Repeated rotation must not overlap the player: \(frame), \(String(describing: playback.geometry.frame))")
                        XCTAssertEqual(observedSelection, selection)
                    }
                }
            }
        ...
        if withChrome {
            ...
            .tabViewBottomAccessory {
                HStack {
                    ...
                    .onGeometryChange(for: CGRect.self) { $0.frame(in: .global) } action: { playback.geometry.update($0, scale: 3) }
            }
            .tabBarMinimizeBehavior(.never)
            .tint(theme.palette.rose)
            .environment(\.playlistPlayerGeometry, playback.geometry)
        } else { shelf }
    }
