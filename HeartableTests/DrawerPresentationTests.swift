...
final class DrawerPresentationTests: XCTestCase {
    ...
    private func render(themeKey: String) async throws {
        ...
        XCTAssertNil(sheet.sheetPresentationController?.preferredCornerRadius,
                     "Use iOS curvature instead of overriding the phone's corner geometry")
...
private struct DrawerFixture: View {
    ...
    @State private var selected: Set<ProviderID> = [.spotify, .apple]
    private let ids = LibrarySearchScope.selectableProviders(connected: [.apple, .spotify, .plex, .jellyfin])
    var body: some View {
