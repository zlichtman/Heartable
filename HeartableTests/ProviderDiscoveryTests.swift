...
final class ProviderDiscoveryTests: XCTestCase {
    ...
    func testSearchMenuExcludesHeartableAndRadioWithoutDuplicateServices() {
        let choices = LibrarySearchScope.selectableProviders(connected: [.apple, .spotify, .apple, .heartable, .wsum])
        XCTAssertEqual(choices, [.apple, .spotify, .audius, .deezer])
        XCTAssertFalse(choices.contains(.heartable))
        XCTAssertFalse(choices.contains(.wsum))
    }
