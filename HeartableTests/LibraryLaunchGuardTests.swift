...
final class LibraryLaunchGuardTests: XCTestCase {
    ...
    func testCrashDuringPlaylistIndexDecodeClearsOnlyTheIndex() {
        var cleared = 0
        var indexCleared = 0
        _ = LibraryLaunchGuard.prepareForLaunch(defaults: defaults, currentBuild: "80", removeCaches: { cleared += 1 }, removePlaylistIndex: { indexCleared += 1 })
        LibraryLaunchGuard.beginBootstrap(defaults: defaults)
        LibraryLaunchGuard.finishBootstrap(defaults: defaults)
        LibraryLaunchGuard.beginPlaylistIndexDecode(defaults: defaults)
        // No finishPlaylistIndexDecode: the process was killed inside the index decode.
        let relaunch = LibraryLaunchGuard.prepareForLaunch(defaults: defaults, currentBuild: "80", removeCaches: { cleared += 1 }, removePlaylistIndex: { indexCleared += 1 })
        XCTAssertEqual(relaunch, .clearedPlaylistIndexAfterAbnormalEnd)
        XCTAssertEqual(cleared, 0, "The browse cache and master library survive")
        XCTAssertEqual(indexCleared, 1)
        XCTAssertFalse(defaults.bool(forKey: LibraryLaunchGuard.playlistIndexMarkerKey))
    }
