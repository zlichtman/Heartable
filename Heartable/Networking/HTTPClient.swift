...
enum HTTPClient {
    ...
    private static func retryAfterDelay(_ resp: HTTPURLResponse) -> TimeInterval? {
        RetryAfter.seconds(resp.value(forHTTPHeaderField: "Retry-After"), cap: maxRetryAfter)
    }
    ...
    private static func nanoseconds(_ seconds: TimeInterval) -> UInt64 {
        // UInt64(NaN or an overflowing value) traps.
        guard seconds.isFinite, seconds > 0 else { return 0 }
        return UInt64(min(seconds, 3_600) * 1_000_000_000)
    }
}

/// One parser for every `Retry-After` header. A server controls this value:
/// "nan", "inf", "1e400" or an HTTP-date far in the future must never reach an
/// `Int(...)`/`UInt64(...)` conversion or a `Duration`, which trap.
enum RetryAfter {
    /// The longest cooldown Heartable will honor or persist.
    static let maximum: TimeInterval = 24 * 60 * 60

    /// Finite seconds in `0...cap`, or nil when the header is absent/unreadable.
    static func seconds(_ raw: String?, now: Date = Date(), cap: TimeInterval = maximum) -> TimeInterval? {
        guard let raw else { return nil }
        let value = raw.trimmingCharacters(in: .whitespaces)
        if let seconds = TimeInterval(value) {
            return clamp(seconds, cap: cap)
        }
        ...
        if let date = formatter.date(from: value) {
            return clamp(date.timeIntervalSince(now), cap: cap)
        }
        return nil
    }

    static func clamp(_ seconds: TimeInterval, cap: TimeInterval = maximum) -> TimeInterval? {
