...
struct LastfmProvider: MusicProvider {
    ...
    private static func mapTrack(_ t: LastfmTrack) -> UnifiedTrack {
        ...
        return UnifiedTrack(
            ...
            // A server string like "99999999999999999" must not overflow.
            durationMs: durationSeconds.multipliedReportingOverflow(by: 1000).overflow
                ? 0 : max(0, durationSeconds * 1000)
        )
    }
