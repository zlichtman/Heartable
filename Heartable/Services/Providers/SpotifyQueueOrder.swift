...
enum SpotifyQueueOrder {
    ...
    static func verify(token: String, deviceID: String?, initialState: PlaybackState? = nil) async throws -> Verification {
        var target = deviceID
        var latest: PlaybackState?
        var refusal: String?
        let firstPoll: SpotifyAPI.PlaybackPoll
        if let initialState { firstPoll = .state(initialState) }
        else { firstPoll = await SpotifyAPI.pollPlayback(token: token) }
        if case .state(let state) = firstPoll {
