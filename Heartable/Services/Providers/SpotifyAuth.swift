...
/// Refreshable Spotify authorization: native SDK when installed, PKCE web fallback.
///
...
enum SpotifyAuth {
    ...
    static func signIn() async throws {
        ...
        if let native = try await SpotifyNativeSignIn.shared.signIn(
            clientID: clientID, scopes: scopes, ownerID: ownerID
        ) {
            guard AccountSessionStore.currentOwnerID == ownerID,
                  await RefreshGate.shared.isCurrent(vaultGeneration, for: ownerID) else {
                throw ProviderError("Your Heartable session changed. Connect Spotify again.")
            }
            await saveSession(TokenResponse(accessToken: native.accessToken,
                refreshToken: native.refreshToken, expiresIn: native.expiresIn),
                fallbackRefresh: nil, ownerID: ownerID, vaultGeneration: vaultGeneration)
            return
        }
...
private final class WebAuth: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private func run(url: URL, scheme: String) async throws -> URL {
        ...
        return try await withCheckedThrowingContinuation { cont in
                ...
                if let callbackURL {
                    ...
                    if nsError.domain == ASWebAuthenticationSessionErrorDomain,
                       nsError.code == ASWebAuthenticationSessionError.canceledLogin.rawValue {
                        resume(.failure(ProviderError("Spotify connection was cancelled.")))
                    } else {
                        resume(.failure(ProviderError(error.localizedDescription)))
                    }
                } else {
                    resume(.failure(ProviderError("Spotify sign-in failed.")))
                }
            }
            ...
            if !session.start() {
                resume(.failure(ProviderError("Spotify sign-in couldn’t open. Try connecting again.")))
            }
        }
    }
