...
struct ProviderCatalogEntry: Identifiable, Sendable {
    ...
    var requiresAccountConnection: Bool {
        status == .live && [.spotify, .apple, .plex, .jellyfin, .lastfm].contains(id)
    }
...
enum ProviderCatalog {
    static let all: [ProviderCatalogEntry] = [
        ...
        .init(.audius, "Audius", 0x7e1bcc, "infinity",
              "Playback compatibility for previously saved Audius tracks.", .live,
              caps: [.playback],
        ...
        .init(.deezer, "Deezer", 0xa238ff, "headphones",
              "Preview playback for previously saved Deezer tracks.", .live,
              caps: [.playback],
        ...
        .init(.wsum, "WSUM", 0xc62836, "dot.radiowaves.left.and.right",
              ...
              playbackTier: .full,
              usesLocalAudioEngine: true,
              adapter: { RadioProvider(id: .wsum) }),
        .init(.radioBrowser, "Radio", 0xc62836, "dot.radiowaves.left.and.right",
              "Live stations streamed in-app.", .live, caps: [.playback],
              playbackTier: .full, usesLocalAudioEngine: true,
              adapter: { RadioProvider(id: .radioBrowser) }),
    ...
    static var searchableLibraryIDs: [ProviderID] {
        all.filter { $0.status == .live && $0.section == .library && $0.capabilities.contains(.search) }.map(\.id)
    }
