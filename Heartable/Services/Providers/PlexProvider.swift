...
struct PlexProvider: MusicProvider {
    ...
    func connectionMetadata() async -> [String: String] {
        struct Identity: Decodable { let username: String?; let title: String?; let thumb: String? }
        guard let owner = AccountSessionStore.currentOwnerID,
              let token = AccountSessionStore.keychainValue(forKey: Self.keyToken),
              let url = URL(string: "https://plex.tv/api/v2/user"),
              let identity = try? await HTTPClient.getJSON(url, headers: Self.plexHeaders(token: token)) as Identity,
              AccountSessionStore.currentOwnerID == owner else { return [:] }
        var result: [String: String] = [:]
        if let name = [identity.title, identity.username].compactMap({ $0?.trimmingCharacters(in: .whitespacesAndNewlines) }).first(where: { !$0.isEmpty }) {
            result["display_name"] = name
        }
        if let photo = identity.thumb, let components = URLComponents(string: photo),
           components.scheme == "https", components.query == nil {
            result["avatar_url"] = photo
        }
        return result
    }
...
private final class PlexWebAuth: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private func run(url: URL, scheme: String) async throws -> URL {
        ...
        return try await withCheckedThrowingContinuation { cont in
            // `start()` can report a failure through the handler *and* return
            // false, so both paths go through one resume-once gate.
                ...
                if let callbackURL {
                    ...
                    if nsError.domain == ASWebAuthenticationSessionErrorDomain,
                       nsError.code == ASWebAuthenticationSessionError.canceledLogin.rawValue {
                        resume(.failure(ProviderError("Plex connection was cancelled.")))
                    } else {
                        resume(.failure(ProviderError(error.localizedDescription)))
                    }
                } else {
                    resume(.failure(ProviderError("Plex sign-in failed.")))
                }
            }
            session.presentationContextProvider = self
            session.prefersEphemeralWebBrowserSession = false
            self.session = session
            // When the session cannot be shown its completion handler never
            // runs; answer here so Connect does not spin forever.
            if !session.start() {
                resume(.failure(ProviderError("Plex sign-in couldn’t open. Try connecting again.")))
            }
        }
    }
