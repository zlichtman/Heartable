...
final class ProvidersStore {
    ...
    private(set) var profileDefaultsGeneration = 0
    ...
    func connect(_ id: ProviderID) async throws {
        ...
        let requestID = lifecycleID
        let provider = ProviderRegistry.provider(for: id)
        MainThreadStallMonitor.note("connect \(id.rawValue) start")
        try await provider.connect()
        MainThreadStallMonitor.note("connect \(id.rawValue) authorized")
    ...
    private func recordConnected(
        ...
        await seedProfile(ownerID: ownerID, requestID: requestID)
    ...
    private func persist(manifest: [ProviderConnectionDTO], requestID: UUID) async {
        for row in manifest {
            guard lifecycleID == requestID,
                  ...
                  let id = ProviderID(rawValue: row.providerId),
                  ProviderCatalog.entry(id)?.requiresAccountConnection == true else { continue }
            // Backfill identity for existing pairings, retaining metadata offline.
            var metadata = row.metadata
            if row.connected {
                let fresh = await ProviderRegistry.provider(for: id).connectionMetadata()
                guard lifecycleID == requestID, AccountSessionStore.currentOwnerID == row.userId else { return }
                metadata.merge(fresh) { _, new in new }
            }
            try? await BackendAPI.shared.upsertProviderConnection(
                providerId: id, connected: row.connected, metadata: metadata, userID: row.userId
            )
        }
        if let ownerID { await seedProfile(ownerID: ownerID, requestID: requestID) }
    }

    private func seedProfile(ownerID: UUID, requestID: UUID) async {
        guard lifecycleID == requestID, self.ownerID == ownerID else { return }
        do {
            try await BackendAPI.shared.seedProfileFromFirstConnection(userID: ownerID)
            guard lifecycleID == requestID, self.ownerID == ownerID else { return }
            profileDefaultsGeneration &+= 1
        } catch { /* Retry during the next account restoration; pairing stays usable. */ }
    }
