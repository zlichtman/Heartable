...
actor DiagnosticsStore {
    ...
    func recordProviderFailure(code: String, summary: String) {
        let payload = "{\"provider\":\"spotify\",\"operation\":\"playlist_items\",\"code\":\"\(code)\"}"
        // Indexing many refused playlists must not evict the actual crash logs.
        if entries().contains(where: { $0.kind == "provider" && $0.payloadJSON == payload
            && Date().timeIntervalSince($0.receivedAt) < 3600 }) { return }
        record(kind: "provider", build: Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "unknown",
