...
final class PlayerStore {
    ...
    private(set) var now: Now? {
        didSet { refreshNowSummary() }
    }
    /// `now` without its playback position. It changes only when the track,
    /// its metadata or its play/pause state changes, never on a progress poll.
    /// The app shell and other chrome observe this instead of `now`, so a
    /// position tick redraws only the views that actually draw progress.
    private(set) var nowSummary: Now?
    ...
    var hasTrack: Bool { nowSummary != nil }
    var hasPendingSpotify: Bool { pendingSpotifyTrack != nil }

    private func refreshNowSummary() {
        var summary = now
        summary?.positionMs = 0
        if summary != nowSummary { nowSummary = summary }
    }
    ...
    private var nativeStartConfirmed = false
        ...
        if let pendingStartTrack,
           ...
           Date() < pendingStartExpiresAt {
            ...
            if targetObserved {
                ...
                pending.isPlaying = nativeStartConfirmed && !userPaused
                candidates.append(pending)
            }
        } else {
            pendingStartTrack = nil
    ...
    func play(tracks: [UnifiedTrack], startingAt: Int? = nil,
        ...
        queue = PlaybackQueue(tracks: tracks, startingAt: startingAt, mode: mode)
        MainThreadStallMonitor.note("play \(queue.current?.providerID.rawValue ?? "none") (queue \(tracks.count))")
            ...
            case .spotify:
                ...
                do {
                    ...
                    try await PlaybackStartupRetry.finishNativeHandoff(
                        wake: { try await SpotifyAppRemote.shared.wakeAndPlay(track) },
                        started: {
                            guard self.playRequestID == requestID else { return }
                            var confirmed = Self.optimisticNow(track)
                            confirmed.isPlaying = true
                            self.now = confirmed
                            self.nativeStartConfirmed = true
                            self.pendingStartExpiresAt = Date().addingTimeInterval(10)
                            self.startingTrackKey = nil
                        },
                        configureQueue: {
                            // The SDK already started this song. Do not start it
                            // again through Connect while the phone is appearing.
                            let following = Array(segment.dropFirst().prefix(50)).map(\.uri)
                            try await SpotifyAppRemote.shared.installQueue(
                                following: following, positionMs: positionMs)
                        },
                        queueFailed: {
                            guard self.playRequestID == requestID else { return }
                            self.showFeedback("Spotify is playing. The remaining Heartable queue couldn’t be set; tap another song to continue.")
                        }
                    )
                }
    ...
    private func installSpotifyQueue(_ tracks: [UnifiedTrack], token: String,
        ...
        // Heartable has already ordered the queue, including ordinary shuffle.
        // Inherited Spotify shuffle/repeat must not reorder or loop that queue.
        guard let selected = tracks.first else { return }
        let state = try await PlaybackStartupRetry.confirmSpotifyPlayback(
            uri: selected.uri, deviceID: resolvedDevice ?? deviceID
        ) { await SpotifyAPI.pollPlayback(token: token) }
        let verification = try await SpotifyQueueOrder.verify(token: token,
            deviceID: resolvedDevice ?? deviceID, initialState: state)
        try Task.checkCancellation()
        if case .unconfirmed(let reason) = verification {
            showFeedback("Spotify is playing, but couldn’t apply the playlist settings. \(reason)")
        }
    }

    func applyPlaybackMode(_ mode: ShuffleMode) async {
        guard let current = now, current.uri == queue.current?.uri else { return }
        queue.reorder(mode: mode)
