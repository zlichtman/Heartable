...
/// Device-level shuffle and listening privacy preferences.
@MainActor
@Observable
final class PlaybackPrefsStore {
    ...
    init(
        ...
        let restoredMode = ShuffleMode.restored(from: raw)
        mode = restoredMode
        defaults.set(restoredMode.rawValue, forKey: Self.modeKey)
    ...
    func order(_ uris: [String]) -> [String] {
        orderForPlayback(uris, mode: mode)
    }
}
