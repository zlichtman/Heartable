...
actor LyricsService {
    ...
    private static func takeLeadingStamp(_ s: inout Substring) -> Int? {
        ...
        // LRC files are user submitted: "[01:.]", "[:]" and absurdly long
        // numbers exist. Reject anything that is not a plausible song offset
        // instead of indexing an empty split or overflowing the arithmetic.
        guard parts.count == 2, parts[0].count <= 4, let minutes = Int(parts[0]),
              (0...999).contains(minutes) else { return nil }
        let secParts = parts[1].split(separator: ".", omittingEmptySubsequences: false)
        guard let secondsText = secParts.first, secondsText.count <= 3,
              let seconds = Int(secondsText), seconds >= 0 else { return nil }
        var ms = (minutes * 60 + seconds) * 1000
        if secParts.count > 1 {
            let frac = String(secParts[1].prefix(3)).padding(toLength: 3, withPad: "0", startingAt: 0)
            if let h = Int(frac), h >= 0 { ms += h }
        }
...
final class LyricsModel {
    ...
    func previewLines(positionMs: Int) -> [String] {
        ...
        return (plain ?? "").split(whereSeparator: \.isNewline)
            ...
            .filter { !$0.isEmpty }.prefix(2).map { $0 }
    }

    nonisolated static func previewIndices(active: Int?, count: Int) -> Range<Int> {
        let start = min(max(0, active ?? 0), max(0, count - 1))
        return start..<min(count, start + 2)
    }
