...
struct PlaybackQueue {
    ...
    init(tracks: [UnifiedTrack] = [], startingAt: Int? = nil,
        ...
        if mode == .order {
            ...
            entries = Self.ordered(original.filter { $0.id != selected?.id }, mode: mode)
            if let selected { entries.insert(selected, at: 0) }
        }
    }

    mutating func reorder(mode: ShuffleMode) {
    ...
    private static func ordered(_ entries: [Entry], mode: ShuffleMode) -> [Entry] {
        mode == .order ? entries : entries.shuffled()
    }
}
