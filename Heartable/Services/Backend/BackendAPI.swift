...
struct BackendAPI: Sendable {
    ...
    func matchContactEmails(hashes: [String]) async throws -> [FoundProfileDTO] {
        ...
        return try await client.rpc("match_contact_emails", params: [
            "p_hashes": AnyJSON.array(hashes.map(AnyJSON.string))
        ]).execute().value
    }

    /// Find profiles via the `find_profile` RPC. Its union can return one person
    /// more than once (matched by name and by a linked handle), and the rows
    /// feed ForEach lists keyed by user, so keep the first row per user.
    func findProfiles(query: String) async throws -> [FoundProfileDTO] {
        ...
        let rows: [FoundProfileDTO] = try await client
            ...
            .value
        var seen = Set<UUID>()
        return rows.filter { seen.insert($0.userId).inserted }
    }
    ...
    func seedProfileFromFirstConnection(userID: UUID) async throws {
        guard await myUID(expected: userID) != nil else { throw BackendError.notSignedIn }
        try await client.rpc("seed_profile_from_first_connection", params: [
            "expected_owner": userID.uuidString
        ]).execute()
    }
    ...
    func updateMixtape(id: UUID,
        ...
        guard let owner = await myUID() else { throw BackendError.notSignedIn }
        var row = MixtapeUpdateDTO(updatedAt: nowISO)
        row.title = title
        row.description = description
        if case let .some(value) = coverUrl { row.coverUrl = value }
        let rows: [IdRowDTO] = try await client.from("mixtapes")
            .update(row)
            .eq("id", value: id.uuidString)
            .eq("owner", value: owner.uuidString)
            .select("id")
            .execute()
            .value
        guard rows.contains(where: { $0.id == id }) else {
            throw BackendError.message("This mixtape could not be saved. Refresh and try again.")
        }
    }

    func deleteMixtape(id: UUID) async throws {
        guard let owner = await myUID() else { throw BackendError.notSignedIn }
        struct DeletedRow: Decodable { let id: UUID }
        let rows: [DeletedRow] = try await client.from("mixtapes")
            .delete()
            .eq("id", value: id.uuidString)
            .eq("owner", value: owner.uuidString)
            .select("id")
            ...
            .value
        guard rows.contains(where: { $0.id == id }) else {
            throw BackendError.message("This mixtape could not be deleted. Refresh and try again.")
        }
    }
    ...
    func addMixtapeTrack(mixtapeID: UUID,
        ...
        // The picker reuses this ID after an uncertain response. Retrying an
        // already committed insert must not append a duplicate or overwrite notes.
        try await client.from("mixtape_tracks")
            .upsert(payload, onConflict: "id", ignoreDuplicates: true).execute()
    }
    ...
    func updateMixtapeTrack(id: UUID,
        ...
        let rows: [IdRowDTO] = try await client.from("mixtape_tracks")
            .update(row)
            .eq("id", value: id.uuidString)
            .select("id")
            .execute()
            .value
        guard rows.contains(where: { $0.id == id }) else {
            throw BackendError.message("This song could not be updated. Refresh and try again.")
        }
    }

    func deleteMixtapeTrack(id: UUID) async throws {
        let rows: [IdRowDTO] = try await client.from("mixtape_tracks")
            .delete()
            .eq("id", value: id.uuidString)
            .select("id")
            ...
            .value
        guard rows.contains(where: { $0.id == id }) else {
            throw BackendError.message("This song could not be removed. Refresh and try again.")
        }
    }
    ...
    func reorderMixtapeTracks(_ ordered: [(id: UUID, position: Int)]) async throws {
        for o in ordered {
            try await updateMixtapeTrack(id: o.id, position: o.position)
        }
    }

    func manageMixtapeLink(id: UUID, action: String) async throws -> MixtapeLinkStatus {
        try await client.rpc("manage_mixtape_link", params: [
            "p_id": id.uuidString, "p_action": action
        ]).execute().value
    }
    ...
    func needsInitialBackup(userID: UUID) async throws -> Bool {
        ...
        let existing: [IdRowDTO] = try await client.from("library_snapshots")
            ...
            .eq("capture_complete", value: true)
