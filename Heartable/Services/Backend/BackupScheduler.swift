...
/// `runIfDue(after:using:)` after library synchronization and on foreground, and this type decides whether a
...
final class BackupScheduler {
    ...
    func runIfDue(after library: LibrarySessionStore, using repository: PlaylistTracksRepository) async {
        guard library.canStartAutomaticBackup(using: repository),
              !isRunning, !suspended, !Task.isCancelled else { return }
        ...
        do {
            ...
            guard !Task.isCancelled, !suspended,
                  library.canStartAutomaticBackup(using: repository),
                  AccountSessionStore.currentOwnerID == ownerID else { return }
            MainThreadStallMonitor.note("automatic backup start")
            defer { MainThreadStallMonitor.note("automatic backup end") }
            ...
            // The saved snapshot is already complete even if updating the
            // account's baseline marker subsequently fails.
