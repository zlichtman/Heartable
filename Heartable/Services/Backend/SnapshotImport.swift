...
extension BackendAPI {
    ...
    private func insertSnapshot(
        ...
        do {
            ...
            guard AccountSessionStore.currentOwnerID == uid else {
                throw BackendError.notSignedIn
            }
            try Task.checkCancellation()
            // Publish only after every child batch has succeeded. Until this
            // update, history and baseline checks ignore the pending snapshot.
            let published: IdRowDTO = try await client.from("library_snapshots")
                .update(["capture_complete": true])
                .eq("id", value: snapshotID.uuidString)
                .eq("owner", value: uid.uuidString)
                .select("id").single().execute().value
            guard published.id == snapshotID else {
                throw BackendError.message("The backup could not be completed.")
            }
        } catch {
    ...
    private func insertChildren(
        ...
        for pl in playlists {
            ...
            let plInserted: [IdRowDTO] = try await client
                ...
                .execute()
                .value
            guard let plID = plInserted.first?.id else {
                throw BackendError.message("A playlist could not be saved. The backup was not completed.")
            }
