...
enum AccountSessionStore {
    ...
    static func removeLibraryCaches(ownerID: UUID?) {
        ...
        removePlaylistIndexCache(ownerID: ownerID)
    }

    /// The playlist-content index alone: the legacy single file and the
    /// per-playlist directory. Browse cache, master library and top tracks stay.
    static func removePlaylistIndexCache(ownerID: UUID?) {
        let fm = FileManager.default
        let owner = ownerID?.uuidString.lowercased()
        guard let appSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            .appendingPathComponent("Heartable", isDirectory: true),
              let files = try? fm.contentsOfDirectory(at: appSupport, includingPropertiesForKeys: nil) else { return }
        for file in files {
            let name = file.lastPathComponent
            guard name.hasPrefix("playlist-tracks-") else { continue }
            if let owner, !name.contains(owner) { continue }
            try? fm.removeItem(at: file)
        }
    }
}
