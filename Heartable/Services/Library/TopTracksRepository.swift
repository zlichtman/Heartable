...
final class TopTracksRepository {
    ...
    private func loadHeartable(
        ...
        slices[key] = ProviderSlice(
            ...
            // Two stat rows can resolve to one key (for example two Spotify
            // local files of the same length). The list is a ForEach source, so
            // keep the first (highest-ranked) row and merge the counts.
            tracks: Self.uniqueTracks(stats.map(\.track)),
    ...
    private func hydrate(ownerID: UUID) async {
        ...
        if hydratedOwnerID != ownerID {
            if let snapshot {
                for var slice in snapshot.slices {
                    // Caches written by older builds can hold repeated keys,
                    // which would reach a ForEach on every visit.
                    slice.tracks = Self.uniqueTracks(slice.tracks)
