...
final class LibrarySessionStore {
    ...
    private(set) var needsSynchronization = false
    private var finishedSynchronization = false
    ...
    /// A cached first paint is not a completed library load. Automatic backups
    /// wait for the core library and playlist index, including resumed work.
    func canStartAutomaticBackup(using repository: PlaylistTracksRepository) -> Bool {
        finishedSynchronization && cachedDataReady && sceneIsActive
            && !synchronizing && !needsSynchronization
            && !library.loading && !library.loadingLiked && !library.refreshing
            && !library.indexingArtists && library.didBuildArtistIndex
            && library.providerNotice == nil
            && repository.hasResolvedAll(library.playlists)
            && !library.playlists.contains(where: { repository.didFail($0) })
    }
    ...
    /// `onDecoded` fires once the browse cache has been decoded and published,
    /// before the playlist index is read: that is the window in which a stale
    /// browse cache could crash the app, so callers lower the launch guard's
    /// library marker there. `onPlaylistIndexDecode` brackets the playlist index
    /// decode (`true` before, `false` after) for the guard's second marker, so a
    /// termination there discards only the index, never the library.
    func prepareCachedData(
        using playlistTracks: PlaylistTracksRepository,
        onDecoded: (@MainActor () -> Void)? = nil,
        onPlaylistIndexDecode: (@MainActor (Bool) -> Void)? = nil
    ) async {
        ...
        let task = Task {
            ...
            MemoryFootprint.checkpoint("library cache decode start")
            await library.hydrate()
            onDecoded?()
            library.warmVisibleArtwork()
            guard !Task.isCancelled else { return }
            onPlaylistIndexDecode?(true)
            await playlistTracks.hydrate()
            onPlaylistIndexDecode?(false)
            guard !Task.isCancelled else { return }
            await library.restoreArtistIndex(from: playlistTracks)
            await master.hydrate()
            MemoryFootprint.checkpoint("library cache decode end")
        }
    ...
    /// Optional library reconciliation yields when the app is not visible.
    /// Playback has its own lifetime and is not changed by this operation.
    func setActive(_ active: Bool) {
        sceneIsActive = active
        if !active, synchronizationTask != nil {
    ...
    func synchronize(
        ...
        guard sceneIsActive, !Task.isCancelled else { needsSynchronization = true; return }
        ...
        guard lifecycleID == requestedLifecycleID, sceneIsActive, !Task.isCancelled else { return }
        ...
        if let synchronizationTask {
            ...
            let interrupted = synchronizationTask.isCancelled
            guard lifecycleID == requestedLifecycleID, sceneIsActive, !Task.isCancelled else { return }
            if synchronizationID == activeID {
                ...
                synchronizing = false
                finishedSynchronization = !interrupted
                needsSynchronization = interrupted || !playlistTracks.hasResolvedAll(library.playlists)
                    || library.playlists.contains(where: { playlistTracks.didFail($0) })
            }
            if interrupted || Self.shouldRerunSynchronization(
        ...
        let master = master
        synchronizing = true
        finishedSynchronization = false
        synchronizationID = operationID
        synchronizationProviderIDs = requestedProviderIDs

        MainThreadStallMonitor.note("library sync start (\(requestedProviderIDs.count) services)")
        let task = Task {
            await library.loadAll(providers: providers, force: force)
            guard !Task.isCancelled else { return }
            MainThreadStallMonitor.note("library core loaded (\(library.playlists.count) playlists, \(library.likedTracks.count) liked)")
            ...
            await master.adopt(
                library.indexedTracks,
                providerIDs: Set(providers.map(\.id))
            )
            MemoryFootprint.checkpoint("library sync end")
        }
        synchronizationTask = task
        await task.value
        MainThreadStallMonitor.note(task.isCancelled ? "library sync paused" : "library sync end")
        ...
        synchronizationProviderIDs = []
        synchronizing = false
        needsSynchronization = task.isCancelled || !playlistTracks.hasResolvedAll(library.playlists)
            || library.playlists.contains(where: { playlistTracks.didFail($0) })
        finishedSynchronization = !task.isCancelled
    }
    ...
    func removeClearedMusicData(ownerID: UUID, playlistTracks: PlaylistTracksRepository) async {
        ...
        guard lifecycleID == requestID, AccountSessionStore.currentOwnerID == ownerID else { return }
        await master.replaceAfterDataClear(library.indexedTracks)
