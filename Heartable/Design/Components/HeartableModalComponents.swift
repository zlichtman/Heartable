...
private struct HeartableSheetChrome: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            // Paint the presentation itself, including the system-managed
            // bottom safe area, rather than a separately sized background view.
...
struct HeartableDrawer<Content: View>: View {
        ...
        ScrollView {
            content
                ...
                .onGeometryChange(for: CGFloat.self) { geometry in
                    ...
                    guard let next = Self.detentHeight(current: contentHeight, measured: height,
                                                       hasMeasured: hasMeasured) else { return }
                    hasMeasured = true
                    contentHeight = next
                }
        }
    ...
    /// iOS 26 insets a partial-height sheet but lays a full-height one edge to
    /// edge, so content near the screen height rewraps at a different width
    /// whenever the detent crosses that line. Growing immediately but shrinking
    /// only by more than a text line breaks that two-height oscillation.
    static func detentHeight(current: CGFloat, measured: CGFloat, hasMeasured: Bool = true) -> CGFloat? {
        guard measured.isFinite, measured > 0 else { return nil }
        // The first real measurement replaces the placeholder detent exactly.
        if !hasMeasured { return measured }
        if measured > current + 1 { return measured }
        if measured < current - 32 { return measured }
        return nil
    }
}
