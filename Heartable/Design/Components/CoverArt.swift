...
final class ArtworkImageCache {
    ...
    /// Notify only views displaying this URL. Observing the whole cache would
    /// redraw every artwork row whenever any other image finishes loading.
    let updates = PassthroughSubject<URL, Never>()

    private let images = NSCache<NSURL, UIImage>()
    private let loads = SharedLoadScheduler<String, UIImage?>(limit: 4)
    ...
    private init() {
        ...
        // iOS warns shortly before it terminates for memory; every decoded
        // bitmap here can be re-read from the disk cache, so drop them all.
    ...
    func load(_ url: URL, expectedGeneration: Int? = nil, priority: Int = 1) async -> UIImage? {
        guard !Task.isCancelled, expectedGeneration == nil || expectedGeneration == generation else { return nil }
        if let cached = image(for: url) { return cached }
        let current = generation
        return await loads.value(for: "\(current):\(url.absoluteString)", priority: priority) { [weak self] in
            await self?.loadUnscheduled(url, expectedGeneration: current)
        } ?? nil
    }

    private func loadUnscheduled(
        _ url: URL,
        ...
        if let entry = await ArtworkDiskCache.shared.entry(for: url) {
               ...
               expectedGeneration == nil || expectedGeneration == generation {
                ...
                if entry.needsRefresh {
                    Task { [weak self] in
                        ...
                        _ = await self.loads.value(for: "refresh:\(requestedGeneration):\(url.absoluteString)") { [weak self] in
                            await self?.refresh(url, expectedGeneration: requestedGeneration)
                            return nil
                        }
                    }
                }
            ...
            guard requestedGeneration == generation, !Task.isCancelled else { return nil }
            await ArtworkDiskCache.shared.removeEntry(for: url)
        }

        let diskGeneration = await ArtworkDiskCache.shared.currentGeneration()
        guard let data = await fetch(url), !Task.isCancelled,
              let image = await Self.decode(data), !Task.isCancelled,
              requestedGeneration == generation,
        ...
        await withTaskGroup(of: Void.self) { group in
            ...
            while index < concurrency {
                ...
                group.addTask { [weak self] in
                    _ = await self?.load(
                        url,
                        expectedGeneration: prefetchGeneration, priority: 0
                    )
                }
                index += 1
            }
    ...
    func remember(_ image: UIImage, for url: URL, encodedByteCount: Int) {
...
struct CachedArtworkImage<Placeholder: View>: View {
    ...
    var body: some View {
        ...
        .onReceive(ArtworkImageCache.shared.updates) { updatedURL in
            guard updatedURL == url else { return }
            // A previous request can have failed or been cancelled while
            // another screen subsequently filled the same cache entry.
            loadedURL = updatedURL
            imageRevision &+= 1
        }
        .task(id: url) {
            guard let url else {
