...
struct AppearanceView: View {
    ...
    @AppStorage(SongListStyle.storageKey) private var songListStyle: SongListStyle = .classic
    @State private var showingSongStyles = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSongStyles) {
            HeartableChoiceSheet(title: "Song layout", items: SongListStyle.allCases.map {
                HeartableChoiceItem(id: $0.rawValue, icon: $0.symbol, title: $0.title,
                                   isSelected: songListStyle == $0)
            }, onCancel: { showingSongStyles = false }, onSelect: {
                songListStyle = SongListStyle(rawValue: $0.id) ?? .classic
            })
        }
    }

    private var songStyleSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            sectionHeader("SONG LAYOUT")
            Button { showingSongStyles = true } label: {
                HStack(spacing: 12) {
                    Image(systemName: songListStyle.symbol).foregroundStyle(theme.palette.rose)
                    Text(songListStyle.title).font(Typography.medium(15)).foregroundStyle(theme.palette.text)
                    Spacer()
                    Image(systemName: "chevron.up.chevron.down").foregroundStyle(theme.palette.textMuted)
                }
