...
struct ProfileView: View {
    ...
    private var nowPlayingLine: some View {
        HStack(spacing: 7) {
            // The position-free summary: a progress poll must not redraw the tab root.
            Image(systemName: player.nowSummary?.isPlaying == true ? "waveform" : "moon.zzz.fill")
        ...
        .lineLimit(2)
        .fixedSize(horizontal: false, vertical: true)
        .foregroundStyle(player.nowSummary == nil ? theme.palette.textMuted : theme.palette.rose)
    }
