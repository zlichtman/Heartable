...
struct DiagnosticsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                Text("Crash, hang, unresponsive-app, memory, and music-service reports from this iPhone. They stay on this iPhone unless you share one.")
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            Text(label(for: entry.kind))
                                ...
                                .foregroundStyle(entry.kind == "crash" || entry.kind == "stall"
    ...
    private func label(for kind: String) -> String {
        switch kind {
        case "provider": "Music service"
        case "crash": "Crash"
        case "hang": "Hang"
        case "stall": "Unresponsive app"
