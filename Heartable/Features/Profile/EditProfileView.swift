...
struct EditProfileView: View {
    ...
    private var featuredPlaylistsForm: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if loadingPlaylists && playlistChoices.isEmpty {
                ...
                // Lazy: a large library has hundreds of choices, each with artwork.
                let lastKey = orderedPlaylistChoices.last?.key
                LazyVStack(spacing: 0) {
                    ForEach(orderedPlaylistChoices) { playlist in
                        playlistChoiceRow(playlist)
                        if playlist.key != lastKey {
                            Divider()
    ...
    private func loadPlaylistChoices(userID: UUID, force: Bool = false) async {
        ...
        do {
            ...
            // The app shell owns library state and keeps it reconciled; never
            // spin up a second LibraryStore that re-probes providers and
            // rewrites the cache. Cached playlists are the answer here; only an
            // empty catalog asks the shell's synchronizer for a pass.
            if providers.hasRefreshed, librarySession.library.playlists.isEmpty {
    ...
    private func uploadAvatar(
        ...
        do {
            guard let data = try await loadData(),
                  let jpeg = await ImageDownscale.jpegInBackground(from: data) else {
...
enum ImageDownscale {
    /// ImageIO decodes straight to the target size, so a 48 MP camera photo
    /// never becomes a ~200 MB full-resolution bitmap. Orientation is applied;
    /// smaller images keep their size.
    nonisolated static func jpeg(from data: Data, maxEdge: CGFloat = 1024, quality: CGFloat = 0.85) -> Data? {
    ...
    /// The same work off the main actor, for callers in views.
    nonisolated static func jpegInBackground(from data: Data, maxEdge: CGFloat = 1024,
                                             quality: CGFloat = 0.85) async -> Data? {
        await Task.detached(priority: .userInitiated) {
            jpeg(from: data, maxEdge: maxEdge, quality: quality)
        }.value
    }
}
