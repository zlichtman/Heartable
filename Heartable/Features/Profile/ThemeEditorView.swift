...
struct ThemeEditorView: View {
    ...
    private var editorHeader: some View {
        HStack(spacing: 16) {
            Text(isExistingTheme ? "Edit Theme" : "New Theme")
            ...
            Button(action: save) {
                Label("Save", systemImage: "checkmark")
                    ...
                    .overlay(Capsule().stroke(trimmedName.isEmpty ? theme.palette.border : theme.palette.rose, lineWidth: 1))
            }
            .buttonStyle(.plain)
    ...
    private func navBar(_ p: Palette) -> some View {
        HStack {
            navItem("heart.fill", "Heartable", active: false, p)
            Spacer()
            navItem("bubble.left.and.bubble.right.fill", "Chats", active: false, p)
            Spacer()
            navItem("house.fill", "Library", active: true, p)
            Spacer()
            navItem("externaldrive.fill", "Backups", active: false, p)
            Spacer()
