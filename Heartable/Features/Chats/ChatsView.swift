...
struct ChatsView: View {
    ...
    @State private var showingSavedMixtapes = false
    ...
    var body: some View {
        NavigationStack {
            content
                ...
                .navigationDestination(item: $createdMixtapeID) { id in
                    MixtapeEditorView(mixtapeID: id)
                        .toolbar(.visible, for: .navigationBar)
                }
                .sheet(isPresented: $showingMixtapeComposer) {
                    SharedMixtapeComposerSheet(friendID: nil, friendName: "someone") {
                        createdMixtapeID = $0
                    }
                }
                .sheet(isPresented: $showingSavedMixtapes) {
                    MixtapesView().heartableSheetChrome()
                }
        }
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    Button("Saved mixtapes") { showingSavedMixtapes = true }
    ...
    private var header: some View {
        HeartablePageHeader(tab: .chats, action: .init(
            title: "Create mixtape", symbol: "plus", perform: { showingMixtapeComposer = true }, iconOnly: true
        ))
    }
