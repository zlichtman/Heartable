...
struct ConversationView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingMixtapeComposer) {
            SharedMixtapeComposerSheet(friendID: friend.userId, friendName: friendName) {
                createdMixtapeID = $0
            }
        }
        .navigationDestination(item: $createdMixtapeID) { id in
            MixtapeEditorView(mixtapeID: id, recipientName: friendName)
    ...
    private var navBar: some View {
        HStack(spacing: 12) {
            ...
            Text(friendName)
                ...
                .lineLimit(1)
            Spacer()
            Button { showingMixtapeComposer = true } label: {
                Image(systemName: "plus").frame(width: 44, height: 44)
            }
            .foregroundStyle(theme.palette.rose)
            .accessibilityLabel("Create mixtape for \(friendName)")
        }
