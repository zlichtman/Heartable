...
struct MiniPlayer: View {
    ...
    var body: some View {
        // The row observes the position-free summary; only `MiniPlayerProgress`
        // follows position ticks, so a poll never re-hosts the native accessory.
        if let now = player.nowSummary {
            HStack(spacing: 10) {
            ...
            .overlay(alignment: .bottom) { MiniPlayerProgress().padding(.horizontal, 14) }
        } else {
            idleRow
        }
    }
    ...
    private var idleRow: some View {
        HStack(spacing: 10) {
            ...
            Spacer(minLength: 8)
            if placement != .inline { PlaybackModeControl(size: 15) }
...
/// The only part of the mini-player that observes playback position.
private struct MiniPlayerProgress: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let duration = player.now?.durationMs ?? 0
            let position = player.now?.positionMs ?? 0
            let pct = duration > 0 ? min(1, max(0, Double(position) / Double(duration))) : 0
