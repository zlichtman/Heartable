...
struct FriendProfileView: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                LazyVGrid(
                    ...
                    ForEach(featuredPlaylists) { playlist in
                        NavigationLink {
                            PlaylistDetailView(playlist: playlist, ownership: .friend)
                        } label: {
    ...
    private func load() async {
        ...
        // Featured playlists render in a ForEach keyed by playlist; a repeated
        // row saved by an older client must not duplicate an identity.
        var seenPlaylists = Set<String>()
        featuredPlaylists = (curation?.playlists.map(\.unified) ?? [])
            .filter { seenPlaylists.insert($0.key).inserted }
