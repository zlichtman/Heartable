...
enum ContactEmailLookup {
    static func hash(_ email: String) -> String? {
        let normalized = email.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard normalized.contains("@"), normalized.count <= 320 else { return nil }
        return SHA256.hash(data: Data(normalized.utf8)).map { String(format: "%02x", $0) }.joined()
    }
}

/// Explicit one-shot lookup. No names, address-book identifiers, hashes, or
/// results are written to disk. Leaving the screen discards the result set.
@MainActor
struct ContactsSection: View {
    ...
    @State private var contacts: [FoundProfileDTO] = []
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ContactsLookupCard(running: running, searched: searched,
                               errorMessage: errorMessage, hasMatches: !contacts.isEmpty) {
    ...
    private func contactRow(_ c: FoundProfileDTO) -> some View {
        NavigationLink(value: FriendRef(userId: c.userId, displayName: c.displayName, avatarUrl: c.avatarUrl)) {
        HStack(spacing: 10) {
            AvatarCircle(urlString: c.avatarUrl, name: c.displayName, size: 42)
            Text(c.displayName ?? "Heartable listener").font(Typography.semibold(14)).foregroundStyle(theme.palette.text).lineLimit(1)
            Spacer(minLength: 4)
            Image(systemName: "chevron.right").foregroundStyle(theme.palette.textMuted)
        }
        .padding(.vertical, 8)
        }.buttonStyle(.plain)
    }
    ...
    private func requestAccess() async {
        ...
        do {
            ...
            guard authStatus == .authorized || authStatus == .limited else {
                if requestID == id {
                    errorMessage = "Contacts access is off. Enable it in Settings to search."
                    running = false
                }
                return
            }
            let hashes = try await Self.fetchContacts()
            guard requestID == id, owner == AccountSessionStore.currentOwnerID else { return }
            let matches = try await BackendAPI.shared.matchContactEmails(hashes: hashes)
            guard requestID == id, owner == AccountSessionStore.currentOwnerID else { return }
            contacts = matches
        } catch {
            guard requestID == id, owner == AccountSessionStore.currentOwnerID else { return }
    ...
    private static func fetchContacts() async throws -> [String] {
        try await Task.detached(priority: .userInitiated) { () throws -> [String] in
            ...
            try store.enumerateContacts(with: request) { contact, _ in
                for email in contact.emailAddresses {
                    if let hash = ContactEmailLookup.hash(email.value as String) { hashes.insert(hash) }
                }
            }
            guard hashes.count <= 5000 else { throw CocoaError(.validationMultipleErrors) }
            return hashes.sorted()
        }.value
    }
}

/// Keep progress, retry and empty results in one card instead of stacking
/// competing empty-state panels. Lookup remains explicitly user initiated.
struct ContactsLookupCard: View {
    @Environment(ThemeStore.self) private var theme
    let running: Bool
    let searched: Bool
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("Find people you know", systemImage: "person.crop.circle.badge.plus")
                .font(Typography.semibold(16))
                .foregroundStyle(theme.palette.text)
            Text("Match contact emails once. Your address book isn’t saved.")
                .font(Typography.body(13))
                .foregroundStyle(theme.palette.textSecondary)
            if let errorMessage, !running {
                Text(errorMessage)
                    .font(Typography.body(13))
                    .foregroundStyle(theme.palette.textSecondary)
                    .accessibilityIdentifier("contacts.error")
            } else if searched && !running && !hasMatches {
                Text("No matches. Phone-only contacts and hidden Apple emails may not match.")
                    .font(Typography.body(13))
                    .foregroundStyle(theme.palette.textSecondary)
            }
            Button(action: onSearch) {
                HStack(spacing: 8) {
                    if running { ProgressView().tint(theme.palette.text) }
                    Text(running ? "Searching…" : errorMessage != nil ? "Try again" : searched ? "Search again" : "Search contacts")
                        .font(Typography.semibold(14))
                }
                .frame(maxWidth: .infinity, minHeight: 44)
                .foregroundStyle(theme.palette.text)
        ...
        .overlay(RoundedRectangle(cornerRadius: Theme.Radius.lg).stroke(theme.palette.border, lineWidth: 1))
    }
}
