...
struct DiscoverView: View {
    ...
    private var topRangeBinding: Binding<StatRange> {
        Binding(get: { topRange }, set: { range in
            // Changing a range is an explicit request within the visible
            // provider, even when that provider was selected automatically.
            // Pin it before the new range's empty cache can trigger fallback.
            topSelection.select(topSource)
            topRange = range
        })
    }
    ...
    private var rangePicker: some View {
        ...
        case .top:
            VStack(alignment: .leading, spacing: 8) {
                ...
                Picker("Range", selection: topRangeBinding) {
