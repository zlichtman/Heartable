...
struct LibraryView: View {
    ...
    var body: some View {
        NavigationStack(path: $navPath) {
            ...
            .onChange(of: store.playlists.map(\.key), initial: true) {
                // Don't erase an account's saved order while a cold catalog is empty.
                if !store.playlists.isEmpty { sortStore.syncCustomOrder(with: store.playlists) }
            }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if browseMode == .artists {
                    artistsSection
                } else if store.loading && store.playlists.isEmpty {
                    loadingRow
                } else {
                    playlistsBody
                }
    ...
    /// Full-width tappable bar for the master Liked Songs list. A separate view:
    /// its song count changes once per liked-songs page during a sync, and that
    /// must redraw only this bar, not the whole Library.
    private var likedBar: some View {
        LikedSongsBar(store: store)
    }
}

private struct LikedSongsBar: View {
    @Environment(ThemeStore.self) private var theme
...
extension LibraryView {
    ...
    private var artistsSection: some View {
        Group {
            if (store.loadingLiked || store.indexingArtists) && store.artists.isEmpty {
                loadingRow
                ...
                LazyVStack(spacing: 0) {
                    ...
                    if store.loadingLiked || store.indexingArtists {
                        HStack(spacing: 8) {
                            ProgressView().tint(theme.palette.rose)
                            Text(store.loadingLiked ? "Loading more artists…" : "Scanning playlists…")
                                .font(Typography.body(12))
                                .foregroundStyle(theme.palette.textSecondary)
                            Spacer()
                        }
