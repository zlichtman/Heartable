...
struct LibrarySearchResultsView: View {
    ...
    private var sourceLabel: String {
        if selectedProviders.isEmpty { return "Services" }
        if selectedProviders.count == 1, let id = selectedProviders.first { return providerName(id) }
        return "\(selectedProviders.count) services"
    }
    private var filteredIsEmpty: Bool {
        tracks.isEmpty && artists.isEmpty && playlists.isEmpty && profiles.isEmpty && stations.isEmpty
    }
    private var stations: [FeaturedRadioStations.Station] {
        guard selectedType == .all || selectedType == .radio else { return [] }
        return FeaturedRadioStations.matching(master.searchTerm)
    }
    ...
    private var filterProviders: [ProviderID] {
        LibrarySearchScope.selectableProviders(connected: connectedProviderIDs)
    }
    ...
    private var filterBar: some View {
        LazyVGrid(columns: filterColumns, spacing: 8) {
            Button {
                ...
                HStack(spacing: 6) {
                    VStack(spacing: 1) {
                        Text("Services").font(Typography.medium(10))
    ...
    private var empty: some View {
        if selectedType == .profiles || selectedType == .radio {
            emptyText("No matches")
        } else if filterProviders.isEmpty {
            emptyText("Connect Spotify, Apple Music, Plex, or Jellyfin to search music.")
        } else if selectedProviders.isEmpty {
            emptyText("Choose a connected service in Search.")
        } else if master.searchedWithNoProviders && selectedType != .profiles {
            emptyText("No matches in your connected services.")
        } else {
            emptyText("No matches")
        }
    }
...
/// for every service that serves it, with source actions available
...
struct MasterTrackRow: View {
    @AppStorage(SongListStyle.storageKey) private var savedListStyle: SongListStyle = .classic
    // Grid and liner-note composition belong to playlist occurrences; mixed
    // provider search retains its source-selection row and uses borderless style.
    private var listStyle: SongListStyle {
        [.covers, .linerNotes].contains(savedListStyle) ? .clean : savedListStyle
    }
    @Environment(ThemeStore.self) private var theme
    ...
    var body: some View {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(track.title)
                        ...
                        .lineLimit(listStyle == .blocks ? 2 : 1)
                    HStack(spacing: 5) {
        ...
        .onLongPressGesture(minimumDuration: 0.4) {
            showingActions = !actionItems.isEmpty
        }
        .sheet(isPresented: $showingActions) {
            HeartableChoiceSheet(
                title: track.title,
                subtitle: "Choose a music service.",
                items: actionItems,
        ...
        .accessibilityHint(
            track.isPlayable
                ? (actionItems.isEmpty ? "Plays track" : "Plays track. More actions available")
                : "No connected playback source"
        )
        .accessibilityActions {
            accessibilityTrackActions
        }
        .modifier(SongListRowSurface(style: listStyle))
    }
