...
final class LibrarySortStore {
    ...
    private static func seededProviderOrder(restored: [ProviderID]) -> [ProviderID] {
        // The saved order feeds a reorder list keyed by provider; a repeated
        // entry from an older build must not duplicate an identity.
        var seen = Set<ProviderID>()
        let unique = restored.filter { seen.insert($0).inserted }
        let seed = [.heartable] + ProviderCatalog.all.map(\.id)
        return unique + seed.filter { !seen.contains($0) }
    }
    ...
    /// A render-time projection. Never mutate observed state or persistence here.
    /// Custom order includes new keys immediately, even before reconciliation.
    func sorted(_ playlists: [UnifiedPlaylist]) -> [UnifiedPlaylist] {
        ...
        case .custom:
            let order = reconciledCustomOrder(with: playlists)
    ...
    func syncCustomOrder(with playlists: [UnifiedPlaylist]) {
        let merged = reconciledCustomOrder(with: playlists)
        if merged != customOrder { setCustomOrder(merged) }
    }

    private func reconciledCustomOrder(with playlists: [UnifiedPlaylist]) -> [String] {
        ...
        var seen = Set<String>()
        return (newOnes + pruned).filter { seen.insert($0).inserted }
    }
