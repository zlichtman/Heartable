...
struct PlaylistDetailView: View {
    ...
    /// Whose library the playlist belongs to. A friend's playlist is browsed
    /// with a one-off read and never written into this account's index.
    enum Ownership { case library, friend }

    let playlist: UnifiedPlaylist
    var ownership: Ownership = .library

    @State private var transientTracks: [UnifiedTrack] = []
    @State private var transientLoading = false
    @State private var transientFailed = false
    ...
    @State private var transientRevision = UUID()

    private var orderingRevision: String {
        "\(playlist.key)|\(playlistTracks.revision(for: playlist))|\(transientRevision)|\(sort)|\(sortReversed)"
    }

    private var tracks: [UnifiedTrack] {
        ownership == .friend ? transientTracks : playlistTracks.tracks(for: playlist)
    }

    private var didFail: Bool {
        ownership == .friend ? transientFailed : playlistTracks.didFail(playlist)
    }

    private var loading: Bool {
        if ownership == .friend { return transientLoading && tracks.isEmpty }
        return tracks.isEmpty
            && !playlistTracks.didFail(playlist)
            && (
                !playlistTracks.hasLoaded(playlist)
                    || playlistTracks.isInitiallyLoading(playlist)
            )
    }

    /// How the visible track rows are ordered (not the playback order).
    private enum TrackSort: String, CaseIterable, Identifiable, Sendable {
    ...
    private var displayedTracks: [UnifiedTrack] { sortedRevision == orderingRevision ? (sortedTracks ?? tracks) : tracks }

    private nonisolated static func ordered(_ tracks: [UnifiedTrack], sort: TrackSort, reversed: Bool) -> [UnifiedTrack] {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .onChange(of: landscape) {
                MainThreadStallMonitor.note(landscape ? "playlist landscape" : "playlist portrait")
            }
        ...
        .onAppear {
            MainThreadStallMonitor.note("open playlist (\(playlist.trackCount) songs, \(ownership == .friend ? "friend" : "library"))")
            PlaylistRotation.setVisible(true, id: rotationID)
        }
        .onDisappear {
            MainThreadStallMonitor.note("close playlist")
            PlaylistRotation.setVisible(false, id: rotationID)
        }
        .tint(theme.palette.text)
    }
    ...
    private var trackList: some View {
        List {
            Section {
                ...
                if loading {
                    ...
                    VStack(spacing: 10) {
                        Text(didFail ? "Couldn't load tracks" : "Empty playlist")
                            .font(Typography.body(14))
                            .foregroundStyle(theme.palette.textSecondary)
                        if didFail {
                            if let message = ownership == .friend ? transientFailureMessage : playlistTracks.failureMessage(for: playlist) {
                                Text(message)
                    ...
                    if listStyle == .covers {
                        // Rows carry their tracks by value: a refresh that shortens
                        // the list must never send a stale index into a subscript.
                        let rows = Array(displayedTracks.prefix(visibleLimit))
                        ForEach(Array(stride(from: 0, to: rows.count, by: 2)), id: \.self) { start in
                            HStack(alignment: .top, spacing: 12) {
                                coverGridSong(rows[start], index: start)
                                if start + 1 < rows.count { coverGridSong(rows[start + 1], index: start + 1) }
                    ...
                    ForEach(Array(displayedTracks.prefix(visibleLimit).enumerated()), id: \.offset) { index, track in
                    ...
                    if visibleLimit < displayedTracks.count {
                        Button("Load more songs") { showMoreRows() }
                            // Grow once per appearance, and only if the row is
                            // still on screen a moment later. A fast fling or a
                            // scrubber drag past it must never cascade through
                            // thousands of rows in one burst.
                            .task {
        ...
        .task(id: orderingRevision) {
            let revision = orderingRevision
            let input = tracks
            let order = sort
            let reversed = sortReversed
            let result = await Task.detached(priority: .userInitiated) {
                Self.ordered(input, sort: order, reversed: reversed)
            }.value
            guard !Task.isCancelled else { return }
            sortedTracks = result
            sortedRevision = revision
            visibleLimit = max(100, min(visibleLimit, result.count))
        }
    ...
    private func showMoreRows() {
        let next = min(visibleLimit + 100, displayedTracks.count)
        if next > visibleLimit { visibleLimit = next }
    }

    private func coverGridSong(_ track: UnifiedTrack, index: Int) -> some View {
        UnifiedTrackRow(track: track, rank: index + 1) {
            let current = displayedTracks
            guard let start = VinylShelfLayout.validSelection(index, count: current.count) else { return }
            coverSelection = start
            Task { await player.play(tracks: current, startingAt: start,
                                     mode: prefs.mode) }
        }
    ...
    private var hero: some View {
        VStack(spacing: 0) {
            ...
            if ownership == .library, playlistTracks.isRefreshing(playlist), !tracks.isEmpty {
                HStack(spacing: 7) {
                    ProgressView()
                        .controlSize(.small)
    ...
    private func load(force: Bool = false) async {
        ...
        case .friend:
            guard force || transientTracks.isEmpty else { return }
            transientLoading = true
            transientFailed = false
            transientFailureMessage = nil
            let read = await playlistTracks.readTransient(playlist)
            guard !Task.isCancelled else { return }
            transientLoading = false
            if case .success(let loaded) = read {
                transientTracks = loaded
                transientRevision = UUID()
            } else {
                transientFailed = true
                transientFailureMessage = read.failureMessage
            }
        }
    }
    ...
    private func playAll(landscape: Bool) async {
        ...
        if landscape, let index = VinylShelfLayout.validSelection(coverSelection, count: displayedTracks.count) {
            await player.play(tracks: displayedTracks, startingAt: index, mode: prefs.mode)
        } else {
            await player.play(tracks: displayedTracks, mode: prefs.mode)
        }
    }
}
