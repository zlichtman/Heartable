...
extension EnvironmentValues {
    @Entry var playlistPlayerGeometry: PlaylistPlayerGeometry? = nil
}

/// Only the landscape viewport reads this measurement. Updating it must never
/// invalidate the TabView that owns the accessory being measured.
@MainActor @Observable
final class PlaylistPlayerGeometry {
    private(set) var frame: CGRect?

    func update(_ proposed: CGRect, scale: CGFloat) {
        guard proposed.minX.isFinite, proposed.minY.isFinite,
              proposed.width.isFinite, proposed.height.isFinite,
              proposed.width > 0, proposed.height > 0 else { return }
        let scale = scale.isFinite && scale > 0 ? scale : 1
        func pixel(_ value: CGFloat) -> CGFloat { (value * scale).rounded() / scale }
        let next = CGRect(x: pixel(proposed.minX), y: pixel(proposed.minY),
                          width: pixel(proposed.width), height: pixel(proposed.height))
...
struct PlaylistVinylViewport<Content: View>: View {
    ...
    @Environment(\.playlistPlayerGeometry) private var playerGeometry
    @ViewBuilder var content: (CGSize) -> Content

    var body: some View {
        GeometryReader { geometry in
            let size = PlaylistViewportBounds.availableSize(
                bounds: geometry.frame(in: .global), player: playerGeometry?.frame)
            content(size)
