...
struct RootView: View {
    ...
    var body: some View {
        ...
        .task(id: providers.profileDefaultsGeneration) {
            guard providers.profileDefaultsGeneration > 0 else { return }
            await me.load(userID: auth.userID, force: true)
        }
        .task(id: auth.userID) {
            ...
            // The decode markers cover only the cache decodes themselves: the
            // library marker for the browse cache, the index marker for the
            // playlist-content index.
            LibraryLaunchGuard.beginBootstrap()
            async let hydration: Void = librarySession.prepareCachedData(
                using: playlistTracks,
                onDecoded: { LibraryLaunchGuard.finishBootstrap() },
                onPlaylistIndexDecode: { began in
                    if began { LibraryLaunchGuard.beginPlaylistIndexDecode() }
                    else { LibraryLaunchGuard.finishPlaylistIndexDecode() }
                }
            )
            ...
            LibraryLaunchGuard.finishPlaylistIndexDecode()
            await profile
            await connections
