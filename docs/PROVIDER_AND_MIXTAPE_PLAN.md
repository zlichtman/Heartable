...
Reviewed September 23, 2026. This separates implemented adapters from proposed work;
...
| Music search | Connected Spotify, Apple Music, Plex, Jellyfin | Each connected service starts selected. Individual toggles only; no All button or public catalog search. |
| Radio | WSUM, KEXP, WFMU | Canonical live station streams and account-scoped station favorites. |
...
Radio includes WSUM 91.7 FM, Freeflow, Sports, KEXP 90.3 FM, and WFMU 91.1 FM.
Station listening is visible as now playing but is not counted as repeated song
listens. There are no show schedules, broadcast archive pages, or broadcast-song
matching jobs. Stations and Heartable profiles are independent search categories.

Stream references: [WSUM](https://wsum.org/),
[KEXP official stream URLs](https://www.kexp.org/mobile/kexp-livestreams/),
[WFMU official streams](https://www.wfmu.org/audiostream.shtml).
KEXP and WFMU returned audio response headers on September 23, 2026.

Audius and Deezer cannot be searched; playback compatibility remains for saved
tracks. Internet Archive, ListenBrainz, and Mixcloud remain inactive. The
`radio_browser` identifier now routes only the curated station allowlist;
arbitrary old station URLs cannot become playback targets.
