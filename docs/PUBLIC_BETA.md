...
- [ ] Order and shuffle playback queue every occurrence; next/previous
...
- [ ] Rotate a playlist with the mini-player visible and without playback. The
      ...
      vinyl song with the full ordered/shuffled playlist queue.
      Repeat at least ten portrait/landscape cycles in both directions, switch
      between playlists of different lengths, and start/pause playback while
      sideways. The cover and ledge must fit immediately without a later size
      correction or overlap, whether the current song belongs to this playlist
      or another one. Simulator layout checks do not replace this device gate.
- [ ] Expand the player from the sideways shelf. Dismiss, device selection,
      seeking, mode, previous/play/next and the lyrics preview must be on-screen
      without scrolling. Test long titles, the largest text setting, plain and
      synced lyrics, live radio, and rotation with the player still presented.
      In both portrait and landscape, the two-line lyrics capsule must remain
      visible without scrolling; advancing synced lines must not change its size.
...
- [ ] Search starts with every connected Spotify, Apple Music, Plex, and Jellyfin
      service individually selected. No All button or disconnected/public sources
      appear. Deselect/reselect sources, change the query, and disconnect a service;
      results and source choices remain coherent. Profiles and stations still work.
- [ ] Play WSUM FM, Freeflow, Sports, KEXP, and WFMU on a physical device. Confirm
      real audio, pause/resume and no manufactured song-play-count entries.
- [ ] Save a station, relaunch and sign out/in: it remains saved for that account
      on that device, but not for a different account. No show/archive requests run.
